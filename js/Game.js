...
class Game extends UI {
  ...
  #placeMinesInCells() {
    ...
    while (minesToPlace) {
      ...
      const hasMineInCell = cell.isMine;
  ...
  #setCellValue(cell) {
    ...
    for (
      ...
      rowIndex <= Math.min(cell.y + 1, this.#numberOfCols - 1);
      rowIndex++
    ) {
      for (
  ...
  #removeCellsEventListeners() {
    this.#cellsElements.forEach((element) => {
      element.removeEventListener("click", this.#handleLeftClick);
      element.removeEventListener("contextmenu", this.#handleRightClick);
    });
  }
}
